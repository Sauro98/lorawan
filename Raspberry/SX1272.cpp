 ...
 *  Version:           1.1
 *  Design:            David Gasc√≥n
 *  Implementation:    Covadonga Albi√±ana & Victor Boria
 */
...
//SetPacketLenght() deleted by Ivano 18/08/2016
...
uint8_t SX1272::setACK()
{
    ...
    {
        // Setting ACK
        // Commented by Ivano 18/08/2016 dst deprecated -> ACK.dst = packet_received.src; // ACK destination is packet source
		// FCTRL TYPE ACK
		ACK.fCtrl = PKT_FCTRL_ACK;
        ACK.type = PKT_TYPE_NO_ACK;
		//Modificato da Ivano 18/08/2016 ora il dst Ë a 32 bit
        ...
        // Writing ACK to send in FIFO Modificato da Ivano 18/08/2016
...
boolean	SX1272::availableData(uint16_t wait)
{
    ...
    { // LoRa mode
        ...
        while( (bitRead(value, 4) == 0) && (millis() - previous < (unsigned long)wait) )
        {
			...
			//Il node mcu non riesce a stare ditro a questo while senza crashare perciÚ aggiungo un millisecondo di delay per farlo respirare
			delay(1);
...
int8_t SX1272::getPacket(uint16_t wait)
{
    ...
    { // FSK mode
        ...
        { // packet received
            ...
            { // CRC correct
                ...
                p_received = true;
#if (SX1272_debug_mode > 0)
                println(F("## Packet correctly received in FSK mode ##"));
#endif
            }
            else
            { // CRC incorrect
                ...
                p_received = false;
#if (SX1272_debug_mode > 0)
                println(F("## Packet incorrectly received in FSK mode ##"));
#endif
            }
        }
        else
        {
#if (SX1272_debug_mode > 0)
            println(F("** The timeout has expired **"));
            println();
#endif
        }
        writeRegister(REG_OP_MODE, FSK_STANDBY_MODE);	// Setting standby FSK mode
    ...
    if( p_received == true )
    {
        ...
        //Commentato da Ivano 18/08/2016 lenght non esiste pi˘ - > packet_received.length = readRegister(REG_RX_NB_BYTES);
...
#if (SX1272_debug_mode > 0)
            println(F("## Packet received:"));
            printf(F("Destination: "));		
            printf(F("Type: "));
            println(packet_received.type);			 	// Printing source
            printf(F("Source: "));
            println(packet_received.src);			 	// Printing source
            printf(F("Packet number: "));
            println(packet_received.packnum);			// Printing packet number
            //printf(F("Packet length: "));
            //println(packet_received.length);			// Printing packet length
            printf(F("Data: "));
            ...
            //println(packet_received.retry);			// Printing number retry
            println(F("##"));
            println();
#endif
            state = 0;
    ...
    else
    {
        ...
        if( (_reception == INCORRECT_PACKET) && (_retries < _maxRetries) )
        {
            ...
            //_retries++;
#if (SX1272_debug_mode > 0)
            println(F("## Retrying to send the last packet ##"));
            println();
#endif
        }
    }
    ...
    if( wait > MAX_WAIT )
    {
        state = -1;
#if (SX1272_debug_mode > 0)
        println(F("** The timeout must be smaller than 12.5 seconds **"));
        println();
#endif
    }
...
*/
//Set destination deleted by Ivano 18/08/2016
...
uint8_t SX1272::setPacket(uint8_t dest, char *payload)
{
    ...
    if( state == 0 )
    {
        ...
        state = 0;
#if (SX1272_debug_mode > 0)
        /*println(F("## Packet set and written in FIFO ##"));
        // Print the complete packet if debug_mode
        println(F("## Packet to send: "));
        printf(F("Destination: "));
        println(packet_sent.dst);			 	// Printing destination
        printf(F("Packet type: "));
        println(packet_sent.type);			// Printing packet type
        printf(F("Source: "));
        println(packet_sent.src);			 	// Printing source
        printf(F("Packet number: "));
        println(packet_sent.packnum);			// Printing packet number
        printf(F("Packet length: "));
        println(packet_sent.length);			// Printing packet length
        printf(F("Data: "));
        for(unsigned int i = 0; i < _payloadlength; i++)
        {
        ...
        println();
        //printf(F("Retry number: "));
        //println(packet_sent.retry);			// Printing retry number
        println(F("##"));*/
#endif
    }
...
uint8_t SX1272::setPacket(uint8_t dest, uint8_t *payload)
{
	...
	//Hardcoded state 0 by Ivano 18/08/2016 Non setto pi˘ la destinazione perciÚ lo stato lo metto a 0 manualmente
	state = 0;
	writeRegister(REG_FIFO_ADDR_PTR, 0x80);  // Setting address pointer in FIFO data buffer
	if (state == 0)
	{
