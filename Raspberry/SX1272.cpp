...
#pragma region constructor_and_setup
...
#pragma endregion Costruttore, PowerOn - PowerOff & helpers


#pragma region packet_helpers_region
/*
...
#pragma endregion Helper per i pacchetti
/*
...
#pragma endregion Ricezione dei pacchetti
...
{
	printf("entering send with timeout\n");
    uint8_t state = 2;
    byte value = 0x00;
    unsigned long previous;
    ...
    { // LoRa mode
		...
		Serial.println("Set transmitting mode");
#if (SX1272_debug_mode > 1)
        value = readRegister(REG_OP_MODE);
...
uint8_t SX1272::sendPacketTimeoutACK(uint8_t dest, uint8_t *payload, uint16_t length16, uint16_t wait)
{
    ...
    {
        state = receive();	// Setting Rx mode to wait an ACK
		Serial.print("state from receive ACK = ");
...
#pragma endregion Spedizione dei pacchetti 
