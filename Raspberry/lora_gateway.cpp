...
#pragma region serial_input_region
	...
	if (Serial.available()) {
		...
		//finchè stai scrivendo su monitor seriale (Serial.avaiable()) e hai scritto meno di 80 caratteri aggiunge il carattere appena inserito alla stringa
		while (Serial.available() && i<80) {
			cmd[i] = Serial.read();
			i++;
			delay(50);
		}
		//alla fine dell'iserimento aggiunge il delimitatore di stringa per far capire alla funzione strlen che quando trova il carattere \0 la stringa è finita
		cmd[i] = '\0';
		...
		if (!e) {
			...
			if (packet.isCommandPacket()) {
				...
				if (c.command != 'w') {
					...
					printf("Sintassi del comando errata\n");
				}
			}
			else {
...
#pragma endregion Esecuzione dei comandi inseriti	  
		...
		//----------ORA SE QUELLO CHE HA RICEVUTO DAL MONITOR SERIALE NON RIENTRA NELLA LISTA DEI COMANDI ALLORA CERCHERà DI INVIARE LA STRINGA COME MESSAGGIO 
...
bool sendDBContent() {
	...
	if (valid) {
		...
		for (int a = 0; a < rows.size(); ++a) {
			...
			const char* time_char = time_string.c_str(); //Se uso time_string.c_str() non funziona, devo per forza usare const char
			struct tm* tm;
